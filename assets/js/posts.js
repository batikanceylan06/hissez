import { getDatabase, ref, onValue, query, orderByChild, equalTo } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-database.js";
  ...
  if (schemaElement) {
    try {
      ...
      const graph = Array.isArray(schema["@graph"])
        ? schema["@graph"].filter((item) => item?.["@type"] !== "BlogPosting")
        : [];
      schemaElement.textContent = JSON.stringify({
        "@context": "https://schema.org",
        "@graph": [...graph, articleSchema]
      });
    } catch (error) {
...
function init() {
  const publishedPostsQuery = query(ref(db, "posts"), orderByChild("status"), equalTo("published"));

  onValue(publishedPostsQuery, (snapshot) => {
