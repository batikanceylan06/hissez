...
function getFormPayload(statusOverride = null) {
  ...
  if (title.length > 120) throw new Error("Başlık en fazla 120 karakter olabilir.");
...
async function savePost(statusOverride = null) {
  ...
  if (!auth.currentUser || !adminAuthorized) {
    ...
    return `
      <article class="admin-post-item">
        ...
        <div class="admin-post-actions">
          ${publicLink}
          <button class="small-btn" type="button" data-action="edit" data-id="${safeId}">Düzenle</button>
          <button class="small-btn" type="button" data-action="publish" data-id="${safeId}">${publishText}</button>
          <button class="small-btn danger" type="button" data-action="delete" data-id="${safeId}">Sil</button>
        </div>
      </article>
    `;
  }).join("");
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!allowed) {
    ...
    await signOut(auth);
    forceLoginOnly();
    showNotice(loginNotice, "error", "Bu panele erişim yetkin yok. Yönetici hesabıyla giriş yapmalısın.");
    return;
  }
